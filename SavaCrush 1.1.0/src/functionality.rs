use ggez::{ graphics::{self, Mesh}, mint::Point2, Context, GameResult };
use rand::{ Rng, rng };
use crate::{assets::{Rgbcolor, COORDS, FIELD_AREA, GAPS, GEM_SCALE, SIDE_LEN } };
use crate::helper_funcs::{ index_to_coords, swap_with };
use crate::objects::{ GameObject::{ self, AnyGem, Null, Explosive1, Explosive2, Explosive3 }, Gem,
Firework, Bomb, Lightning, Void };

pub fn background(context: &mut Context) -> GameResult<Mesh> {
	return Mesh::new_rectangle(
        context, graphics::DrawMode::fill(), graphics::Rect::new(0., 0., 
        SIDE_LEN, SIDE_LEN), Rgbcolor::Field.ggez_color());
}
...
pub fn vertical_outline(context: &mut Context) -> GameResult<Mesh> {
    ...
    for i in 0..=GAPS {
        ...
        if i % 2 == 0 {
            vertical_points.extend_from_slice(&[Point2 { x: pos, y: 0. },
                Point2 { x: pos, y: SIDE_LEN }]);
        } else {
            vertical_points.extend_from_slice(&[Point2 { x: pos, y: SIDE_LEN },
                Point2 { x: pos, y: 0. }]);
        }
    }
    let vertical_mesh: Mesh = Mesh::new_line(
        context, &vertical_points, 2., Rgbcolor::Border.ggez_color())?;
...
pub fn horizontal_outline(context: &mut Context) -> GameResult<Mesh> {
    ...
    for i in 0..=GAPS {
        let pos: f32 = (GEM_SCALE * i) as f32;
        if i % 2 == 0 {
            horizontal_points.extend_from_slice(&[Point2 { x: 0., y: pos },
                Point2 { x: SIDE_LEN, y: pos }]);
        } else {
            horizontal_points.extend_from_slice(&[Point2 { x: SIDE_LEN, y: pos },
                Point2 { x: 0., y: pos }]);
        }
    }
    let horizontal_mesh: Mesh = Mesh::new_line(
        context, &horizontal_points, 2., Rgbcolor::Border.ggez_color())?;
...
pub fn selection(context: &mut Context, idx: Option<usize>) -> GameResult<Mesh> {
	...
	return graphics::Mesh::new_rectangle(
          context, graphics::DrawMode::stroke(3.), graphics::Rect::new(
			coordinates.0, coordinates.1, 
        	GEM_SCALE as f32, GEM_SCALE as f32), Rgbcolor::Selection.ggez_color());
}
...
pub fn generate_gems(board: &mut [GameObject; FIELD_AREA]) -> () {
    let mut thread: rand::prelude::ThreadRng = rng();
    for idx in 0..GAPS as usize {
        if board[idx].is_void() {
            board[idx] = AnyGem(Gem::new(thread.random_range(0..=7), idx));
        }
    }
}
...
pub fn destroy_gems(board: &mut [GameObject; FIELD_AREA]) -> () {
	for m in (3..=5).rev() {
		let row_check_generator: Vec<usize> = (0..board.len()).filter(|num: &usize| {
			let modulus: usize = (*num + 1) % GAPS;
			match m {
				5 => ![0, GAPS - 1, GAPS - 2, GAPS - 3]
				.contains(&modulus),
				4 => ![0, GAPS - 1, GAPS - 2].contains(&modulus),
				_ => ![0, GAPS - 1].contains(&modulus),
			}
		}).map(|num: usize| num as usize).collect();
		for idx in row_check_generator { // row checks
			if !board[idx].is_void() && ((1..m).all(|x: usize| board[idx] == board[idx + x])) && ((1..m)
			.all(|x: usize| COORDS[idx].y == COORDS[idx + x].y)) {
				for i in 0..m {
		...
		let col_check_generator: Vec<usize> = (0..(board.len() - GAPS * (m - 1)))
		.collect();
		for idx in col_check_generator { // column check
...
pub fn check_matches(mut board: [GameObject; FIELD_AREA]) -> bool {
	for object in board.iter() {
		if object.is_explosive() {
			return true;
		}
	}
...
pub fn legit_swap(mut board: [GameObject; FIELD_AREA], idx1: usize, 
    idx2: usize) -> bool {
	let (row1, col1, row2, col2) = (idx1 % GAPS + 1, idx1 / GAPS + 1,
	idx2 % GAPS + 1, idx2 / GAPS + 1);
	if (row1 == row2 && col1 == col2 - 1) || (row1 == row2 && col1 == col2 + 1) ||
	(row1 == row2 - 1 && col1 == col2) || (row1 == row2 + 1 && col1 == col2)  {
