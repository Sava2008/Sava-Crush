...
use ggez::{self, conf, event, graphics::{ self, Mesh }, 
input::mouse::{ self, button_pressed, position }, mint::Point2, Context, ContextBuilder, GameResult };
use std::env::current_dir;
use std::time::{ Instant, Duration };

mod assets; mod functionality; mod objects; mod helper_funcs;
use assets::{ WIN_SCALES, GAPS, Rgbcolor, COORDS, SIDE_LEN, Mode, MainState, Phase, Swap, Sounds };
use functionality::{ background, horizontal_outline, vertical_outline, selection, generate_gems, gravity, 
	destroy_gems, gems_positions, legit_swap, check_matches, shuffle };
use helper_funcs::{ get_index_of, load_images, generate_board, swap_with, explosive_index, 
	define_swap_direction };
...
impl MainState {
	...
	fn perform_swapping(&mut self, context: &mut Context) -> GameResult {
		...
		if legit_swap(self.board, idx1, idx2) {
			...
			let explosive_idx:(Option<usize>, usize)  = explosive_index(&self.board,
				idx2, idx1);
			if let Some(i) = explosive_idx.0 {
				match self.board[i].get_placeholder() {
					8 => { GameObject::explode(i, 
							define_swap_direction(idx1 as isize, idx2 as isize), 
							&mut self.board, explosive_idx.1,
							&mut self.sounds, context)?; },
					9 => { GameObject::explode(i, 
							Swap::Horizontal, &mut self.board, explosive_idx.1,
							&mut self.sounds, context)?; },
					10 => { GameObject::explode(i, 
							Swap::Horizontal, &mut self.board, explosive_idx.1,
							&mut self.sounds, context)?; },
					_ => (),
				}
		...
		if !self.any_nones_on_board {
			...
			if self.mode == Mode::Selection {
				match self.phase {
					...
					Phase::MatchCheck => {  if !check_matches(self.board) {
...
impl event::EventHandler for MainState {
	fn update(&mut self, context: &mut Context) -> GameResult {
		...
		if current - self.last_update >= self.update_interval {
			if self.board[0..GAPS].iter().any(|el: &GameObject | el.is_void()) {
				generate_gems(&mut self.board);
			}
	...
	fn draw(&mut self, context: &mut Context) -> GameResult {
		...
		graphics::draw(context, &self.horizontal_outline_mesh, 
			graphics::DrawParam::default())?;
		graphics::draw(context, &self.vertical_outline_mesh, 
			graphics::DrawParam::default())?;
		for (i, gem) in gem_indices {
			graphics::draw(context, &self.all_images[gem], 
				graphics::DrawParam::default().dest(COORDS[i])
			.scale([0.25, 0.25]))?;
		}
...
fn main() -> GameResult {
	...
	.add_resource_path(current_dir()?.join("assets")).build()?;
