use ggez::{ graphics::{ GlBackendSpec, Image, ImageGeneric }, mint::Point2, Context };
use crate::assets::{ FIELD_AREA, GAPS, GEM_SCALE, Swap };
use crate::objects::{ GameObject::{ self, Null }, Void } ;

pub fn index_to_coords(index: usize) -> (f32, f32) {
    return (((index % GAPS) * GEM_SCALE) as f32, ((index / GAPS) * GEM_SCALE) as f32);
}
...
pub fn load_images(context: &mut Context) -> [ImageGeneric<GlBackendSpec>; 11] {
    return [Image::new(context, "/Images/Blue_gem.png").unwrap(),
...
pub fn generate_board() -> [GameObject; FIELD_AREA] {
	return (0..FIELD_AREA).map(|index: usize| Null(Void::new(index))).collect();
}
...
pub fn explosive_index(board: &[GameObject; FIELD_AREA],
	idx1: usize, idx2: usize) -> (Option<usize>, usize) {
