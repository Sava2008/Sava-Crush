use crate::assets::{FIELD_AREA, GAPS, GEM_SCALE, Sounds, Swap};
use crate::helper_funcs::{generate_board, index_to_coords, indices_of_col, indices_of_row};
use ggez::GameResult;
use rand::{Rng, rng};
use std::cmp::{max, min};
...
impl Firework {
    ...
    pub fn explode(
        &self,
        swap: Swap,
        board: &mut [GameObject; FIELD_AREA],
        other: usize,
        sounds: &mut Sounds,
            ...
            Swap::Horizontal => {
                for x in indices_of_row(coords.1 as usize / GEM_SCALE) {
                    if board[x].is_explosive() && x != self.index {
                        GameObject::explode(x, Swap::Horizontal, board, other, sounds, context)?;
    ...
    pub fn explode(
        ...
        for idx in self.bomb_surroundings() {
            if board[idx].is_explosive() && idx != self.get_idx() {
                GameObject::explode(idx, Swap::Horizontal, board, other, sounds, context)?;
...
impl Lightning {
        ...
        match board[other].get_placeholder() {
            ...
            x if x == 8 || x == 9 => {
                ...
                for _ in 0..=4 {
                    ...
                    (board[self.index], board[other]) = (
                        GameObject::Null(Void::new(self.index)),
    ...
    pub fn all_identical_objects(
        &self,
        board: &[GameObject; FIELD_AREA],
        other_index: usize,
    ) -> Vec<usize> {
        return (0..FIELD_AREA)
            .filter(|index: &usize| board[*index] == board[other_index])
            .collect();
    }
}
...
impl PartialEq for GameObject {
    fn eq(&self, other: &Self) -> bool {
        return match (self, other) {
            (GameObject::AnyGem(obj1), GameObject::AnyGem(obj2)) => {
                obj1.placeholder == obj2.placeholder
            }
            _ => false,
        };
    }
...
impl GameObject {
    ...
    pub fn is_explosive(&self) -> bool {
        return [8, 9, 10]
            .iter()
            .any(|n: &usize| n == &self.get_placeholder());
    }
    ...
    pub fn explode(
        ...
        match board[idx] {
            GameObject::Explosive3(x) => x.explode(swap, board, other, sounds, context)?,
            GameObject::Explosive1(x) => x.explode(swap, board, other, sounds, context)?,
            GameObject::Explosive2(x) => x.explode(swap, board, other, sounds, context)?,
            _ => (),
        };
        return Ok(());
    }
